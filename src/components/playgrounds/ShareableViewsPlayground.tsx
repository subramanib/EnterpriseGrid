...
export function ShareableViewsPlayground() {
  const [shareableViewsEnabled, setShareableViewsEnabled] = useState(true);
...
<DataTable
  ...
  data={data}
  config={{
    shareableViews: true,   // Enable Shareable Views UI
...
/>
  ...
  return (
    <div id="shareable-views" className="mb-16 scroll-mt-24">
      <h2 className="text-2xl font-semibold mb-4 text-slate-800">🔗 Shareable Views</h2>
      <div className="text-slate-600 mb-6 leading-relaxed">
        Enable users to generate a unique, shareable URL that securely encodes the current state of the datatable—including searches, active column visibility, density settings, and filters.
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-6">
        <div className="w-full lg:w-1/2 flex-1 min-w-0 rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm flex flex-col">
          ...
          <div className="p-0 flex-grow relative min-h-[400px]">
            <DataTable 
              id="playground-shareable-views-demo"
              columns={mockColumns} 
              data={mockData} 
              config={{ 
                searchable: true, 
                ...
                shareableViews: shareableViewsEnabled
              }}
              pagination
            />
          </div>
