...
export function BookmarksPlayground() {
  const [bookmarksEnabled, setBookmarksEnabled] = useState(true);

  const codeSnippet = `
<DataTable
  id="playground-bookmarks-demo"
  columns={columns}
  data={data}
  config={{
    bookmarks: true,     // Enable Favorites & Bookmarks feature
    searchable: true,
  }}
/>
  `.trim();
  ...
  return (
    <div id="favorites-bookmarks" className="mb-16 scroll-mt-24">
      <h2 className="text-2xl font-semibold mb-4 text-slate-800">⭐ Favorites & Bookmarks</h2>
      <div className="text-slate-600 mb-6 leading-relaxed">
        Enable users to bookmark important records in the grid with a single click. Filter the grid to show only favorited items instantly via the toolbar or header button, and persist favorites across browser sessions automatically.
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-6">
        <div className="w-full lg:w-1/2 flex-1 min-w-0 rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm flex flex-col">
          <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex justify-between items-center">
            <span className="text-sm font-semibold text-slate-700">Interactive Bookmarks Grid</span>
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-500 font-medium">Bookmarks Feature:</span>
              <button 
                onClick={() => setBookmarksEnabled(!bookmarksEnabled)}
                className={`px-3 py-1 text-xs font-semibold rounded-full transition-colors ${
                  bookmarksEnabled ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
                {bookmarksEnabled ? 'Enabled' : 'Disabled'}
              </button>
            </div>
          </div>
